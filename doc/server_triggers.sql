...
CREATE TRIGGER GAME_UPDT_TRG AFTER UPDATE ON GAME
  FOR EACH ROW BEGIN
    IF NEW.SCORE1 is not NULL AND OLD.SCORE1 IS NULL AND NEW.SCORE2 IS NOT NULL AND OLD.SCORE2 IS NULL THEN
      ...
      END IF;
      update MODIFY_COUNT set MAX_GAMES = NEW.ID where TEAM_ID IN (NEW.TEAM1, NEW.TEAM2);
  END IF;
END 
$$  
  ...
  FOR EACH ROW BEGIN
	...
	update TEAM_PLAYER set AVG_FORM = avgPerformance, GOALS = GOALS + NEW.GOAL, ASSISTS = ASSISTS + NEW.ASSIST, PLAYED = PLAYED + 1
	 ...
	 CASE 
	 	...
	 	WHEN LENGTH(latestForm) < 6 THEN update PLAYER set LATEST_FORM = CONCAT(LATEST_FORM, '/', NEW.FORM) where ID = NEW.PLAYER_ID;
	 	ELSE update PLAYER set LATEST_FORM = CONCAT(SUBSTR(LATEST_FORM, 3), '/', NEW.FORM) where ID = NEW.PLAYER_ID;
	 END CASE;
END 
$$  
